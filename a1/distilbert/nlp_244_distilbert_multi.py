...
class ParserModel(torch.nn.Module):
    def __init__(self,
        ...
        self.distil_bert_model = DistilBertModel.from_pretrained(model_name_or_path)
        self.dropout = Dropout(dropout)
        self.num_intent_labels = num_intent_labels
        self.num_slot_labels = num_slot_labels
        self.intent_classifier = nn.Linear(self.distil_bert_model.config.hidden_size, num_intent_labels)
        self.slot_classifier = nn.Linear(self.distil_bert_model.config.hidden_size, num_slot_labels)
    ...
    def forward(self,
        ...
        last_hidden_states = self.distil_bert_model(input_ids=input_ids,
        ...
        pooler_output = last_hidden_states[0].permute(1, 0, 2)

        intent_logits = self.intent_classifier(self.dropout(pooler_output[0]))
        slot_logits = self.slot_classifier(self.dropout(last_hidden_states[0]))
...
model = ParserModel(model_name_or_path='distilbert-base-uncased',
                    dropout=dropout,
