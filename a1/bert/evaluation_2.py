...
def opener(filename):
    with open(filename, 'r') as f:
        uncleaned_relations = [line.strip().split(' ')
...
if __name__ == '__main__':
    ...
    y_true, y_true_intents = opener(
        'hw1_tags_test.txt') if args.test else opener('hw2_tags_test.txt')
    y_pred, y_pred_intents = opener(
        'prediction.txt') if args.test else opener('q_submission.txt')

    print("Sentence Accuracy is {:.2f}%".format(
        accuracy_score(y_true, y_pred) * 100))
    print("Slot F1 Score is {:.2f}%".format(f1_score(y_true, y_pred) * 100))
    print("Intent F1 Score is {:.2f}%".format(
        f1_score(y_true_intents, y_pred_intents, intent=True) * 100))
